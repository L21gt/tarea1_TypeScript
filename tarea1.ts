...
function soloStrings(valores: (string | number)[]): string[] {
    return valores
...
function dividirTS(a: number, b: number): [number, boolean] {
    if (b === 0) {
        return [0, false];
    } else {
        return [a / b, true];
...
class Usuario {
    constructor(public nombre: string, private edad: number, protected correo: string) {}
...
class Moto extends Vehiculo {
    constructor(marca: string, modelo: string, public cc: number) {
        super(marca, modelo);  // Llama al constructor de la clase base
    }
...
function imprimirArea(f: Figura): string {
    return `Area: ${f.area()}`;
